...
interface VideoData {
  success: boolean;
  type?: "drm" | "drm_no_key" | "hls" | "mp4" | "youtube" | "live";
  mpdUrl?: string;
  hlsUrl?: string;
  rawHlsUrl?: string;
  videoUrl?: string;
...
export default function LiveVideoPlayer({
  ...
  // ─── Load HLS via hls.js (all devices including iOS 17+) ───────────────────
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-2 sm:py-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <h2 className="text-white font-bold text-xs sm:text-[15px] line-clamp-1 opacity-90" title={title}>
            {title.replace(/\s+/g, " ").trim() || "Live Class"}
          </h2>
        </div>
        <button
          onClick={onClose}
                        ...
                        <div className="absolute bottom-full right-0 mb-2 bg-gray-900/95 backdrop-blur-sm border border-white/10 rounded-xl overflow-hidden min-w-[72px] shadow-2xl z-30">
                          ...
                          {qualities.map(q => (
                            <button key={q.index} onClick={(e) => { e.stopPropagation(); setQuality(q.index); }}
                              className={`w-full text-left px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm transition-colors ${currentQuality === q.index ? "text-red-400 bg-red-500/10 font-bold" : "text-white/80 hover:bg-white/10 hover:text-white"}`}>{qualityLabel(q)}</button>
                          ))}
                        </div>
                      )}
                    </div>
